...
void MeshObject::setData(std::vector<tinyobj::shape_t> shapes, Material* mtl)//, const char* filename)
{

	NumVertexes =  (shapes[0].mesh.positions.size() / 3) ;
	int NumNormals =  shapes[0].mesh.normals.size();
	int NumTexts =  shapes[0].mesh.texcoords.size();
	int NumIndices = shapes[0].mesh.indices.size();
	std::cout << "NumVertexes " << NumVertexes << std::endl;
	std::cout << "NumNormals " << NumNormals << std::endl;
	std::cout << " NumTexts " << NumTexts << std::endl;
	std::cout << " Numindex " << NumIndices << std::endl;

	Vertexes = new Vertex[NumVertexes];
	NumTriangles =  shapes[0].mesh.indices.size() / 3;
	...
	{
		Vertexes[i].Position.Set(shapes[0].mesh.positions[3 * i + 0], shapes[0].mesh.positions[3 * i + 1],
				                 shapes[0].mesh.positions[3 * i + 2]);

		Vertexes[i].Normal.Set(shapes[0].mesh.normals[3 * i + 0], shapes[0].mesh.positions[3 * i + 1], 
							   shapes[0].mesh.normals[3 * i + 2]);

		Vertexes[i].TexCoord.Set(shapes[0].mesh.texcoords[2 * i + 0], shapes[0].mesh.texcoords[2 * i + 1], 0);

	//	Vertexes[i].Pixel.Set(color_data[i].getRed(), color_data[i].getGreen(), color_data[i].getBlue());
	}
	...
	for (int i = 0; i < NumTriangles; i++)
	{
		...
		int idx0 = shapes[0].mesh.indices[3 * i + 0];
		int idx1 = shapes[0].mesh.indices[3 * i + 1];
		int idx2 = shapes[0].mesh.indices[3 * i + 2];

		Triangles[i].Init(&Vertexes[idx0], &Vertexes[idx1], &Vertexes[idx2], 0);		
	}
...
bool MeshObject::loadTexture(const char* filename)
{
